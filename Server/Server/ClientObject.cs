        ...
        public void Process()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (cmd < 0 || cmd > 3 || parse == false) // проверяем корректность ввода
                    {
                    ...
                    else if (cmd == 0 || cmd == 1 || cmd == 3) // если необходимо указывать номер службы
                    {
                        ...
                        parse = Int32.TryParse(s, out numberService);

                        if(parse == false) // проверяем корректность ввода
                        {
                            Console.WriteLine("Неверный номер службы");
                            continue;
                        }
                        message += " " + s;
                    }
